...
{
    public sealed class AddRatedShowNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddRatedShowNote()
        {
            ...
            TraktResponse<TraktNote> response = await client.Notes.AddRatedShowNoteAsync(show, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddRatedShowNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            ...
            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddRatedShowNoteAsync(show, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddRatedShowNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var show = new TraktShow { IDs = new TraktShowIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddRatedShowNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<ArgumentNullException>();

            act = () => client.Notes.AddRatedShowNoteAsync(show, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddRatedShowNoteAsync(show, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
