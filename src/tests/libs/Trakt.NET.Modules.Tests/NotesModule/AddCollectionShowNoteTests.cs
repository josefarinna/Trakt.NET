...
{
    public sealed class AddCollectionShowNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddCollectionShowNote()
        {
            ...
            string notes = "Welcome to Pollos Hermanos!";

            TraktResponse<TraktNote> response = await client.Notes.AddCollectionShowNoteAsync(show, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddCollectionShowNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, statusCode);

            var show = new TraktShow { IDs = new TraktShowIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddCollectionShowNoteAsync(show, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddCollectionShowNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var show = new TraktShow { IDs = new TraktShowIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddCollectionShowNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<ArgumentNullException>();

            act = () => client.Notes.AddCollectionShowNoteAsync(show, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddCollectionShowNoteAsync(show, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
