...
{
    public sealed class AddShowNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddShowNote()
        {
            ...
            TraktResponse<TraktNote> response = await client.Notes.AddShowNoteAsync(show, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddShowNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            ...
            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddShowNoteAsync(show, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddShowNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var show = new TraktShow { IDs = new TraktShowIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddShowNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddShowNoteAsync(show, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddShowNoteAsync(show, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
