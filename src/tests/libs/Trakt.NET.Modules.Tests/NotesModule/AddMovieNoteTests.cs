...
{
    public sealed class AddMovieNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddMovieNote()
        {
            ...
            TraktResponse<TraktNote> response = await client.Notes.AddMovieNoteAsync(movie, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddMovieNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            ...
            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddMovieNoteAsync(movie, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddMovieNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var movie = new TraktMovie { IDs = new TraktMovieIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddMovieNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddMovieNoteAsync(movie, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddMovieNoteAsync(movie, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
