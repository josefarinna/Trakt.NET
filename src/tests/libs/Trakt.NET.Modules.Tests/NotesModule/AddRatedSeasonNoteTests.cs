...
{
    public sealed class AddRatedSeasonNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddRatedSeasonNote()
        {
            ...
            string notes = "Hello Gus.";

            TraktResponse<TraktNote> response = await client.Notes.AddRatedSeasonNoteAsync(episode, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddRatedSeasonNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, statusCode);

            var episode = new TraktSeason { IDs = new TraktSeasonIDs { Trakt = 16U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddRatedSeasonNoteAsync(episode, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddRatedSeasonNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var episode = new TraktSeason { IDs = new TraktSeasonIDs { Trakt = 16U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddRatedSeasonNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<ArgumentNullException>();

            act = () => client.Notes.AddRatedSeasonNoteAsync(episode, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddRatedSeasonNoteAsync(episode, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
