...
{
    public sealed class AddEpisodeNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddEpisodeNote()
        {
            ...
            TraktResponse<TraktNote> response = await client.Notes.AddEpisodeNoteAsync(episode, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddEpisodeNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            ...
            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddEpisodeNoteAsync(episode, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddEpisodeNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var episode = new TraktEpisode { IDs = new TraktEpisodeIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddEpisodeNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddEpisodeNoteAsync(episode, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddEpisodeNoteAsync(episode, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
