...
{
    public sealed class AddRatedMovieNoteTests
    {
        private const string AddNoteUri = "notes";

        [Fact]
        public async Task TestAddRatedMovieNote()
        {
            ...
            TraktResponse<TraktNote> response = await client.Notes.AddRatedMovieNoteAsync(movie, notes, cancellationToken: TestContext.Current.CancellationToken);
        ...
        public async Task TestAddRatedMovieNoteThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            ...
            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddRatedMovieNoteAsync(movie, "Notes", cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestAddRatedMovieNoteThrowsArgumentExceptions()
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(AddNoteUri, HttpStatusCode.Created);
            var movie = new TraktMovie { IDs = new TraktMovieIDs { Trakt = 1U } };

            Func<Task<TraktResponse<TraktNote>>> act = () => client.Notes.AddRatedMovieNoteAsync(null!, "Notes");
            await act.ShouldThrowAsync<ArgumentNullException>();

            act = () => client.Notes.AddRatedMovieNoteAsync(movie, null!);
            await act.ShouldThrowAsync<TraktPostValidationException>();

            act = () => client.Notes.AddRatedMovieNoteAsync(movie, string.Empty);
            await act.ShouldThrowAsync<TraktPostValidationException>();
        }
    }
}
