...
namespace TraktNET.SyncModule
{
    public sealed class GetCollectionMoviesTests
    {
        ...
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(GetCollectionMoviesUri, statusCode);

            Func<Task<TraktListResponse<TraktSyncCollectionMovie>>> act = () => client.Sync.GetCollectionMoviesAsync(cancellationToken: TestContext.Current.CancellationToken);
