...
namespace TraktNET.SyncModule
{
    public sealed class GetWatchedShowsTests
    {
        ...
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(GetWatchedShowsUri, statusCode);

            Func<Task<TraktListResponse<TraktWatchedShow>>> act = () => client.Sync.GetWatchedShowsAsync(cancellationToken: TestContext.Current.CancellationToken);
