...
namespace TraktNET.CalendarsModule
{
    public sealed class GetAllFinalesTests
    {
        ...
        [Theory]
        [InlineData("2011-04-18T00:00:00.000Z", 7U, null, "calendars/all/shows/finales/2011-04-18/7", "Calendars\\calendarshowsfinales_minimal.json")]
        [InlineData("2011-04-18T00:00:00.000Z", 7U, TraktExtendedInfo.Full, "calendars/all/shows/finales/2011-04-18/7?extended=full", "Calendars\\calendarshowsfinales.json")]
        ...
        [Fact]
        public async Task TestGetAllFinalesWithFilter()
        {
            ...
            string requestUri = $"{GetAllFinalesUri}/2011-04-18/7?{filter}";
            TraktClient client = ModuleTestUtility.GetClient(requestUri, responseContent);

            TraktListResponse<TraktCalendarShow> response = await client.Calendar.GetAllFinalesAsync(date, 7U, filter, null, TestContext.Current.CancellationToken);
        ...
        public async Task TestGetAllFinalesThrowsApiException(HttpStatusCode statusCode, Type exceptionType)
        {
            ...
            TraktClient client = ModuleTestUtility.GetClient($"{GetAllFinalesUri}/{dateString}/1", statusCode);

            Func<Task<TraktListResponse<TraktCalendarShow>>> act = () => client.Calendar.GetAllFinalesAsync(date, 1U, cancellationToken: TestContext.Current.CancellationToken);
            (await act.ShouldThrowAsync(exceptionType)).ShouldNotBeNull();
        }

        [Fact]
        public async Task TestGetAllFinalesThrowsArgumentException()
        {
            DateTime date = DateTime.UtcNow;
            string dateString = date.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
            TraktClient client = ModuleTestUtility.GetClient($"{GetAllFinalesUri}/{dateString}/1", HttpStatusCode.OK);

            Func<Task<TraktListResponse<TraktCalendarShow>>> act = () => client.Calendar.GetAllFinalesAsync(default, default, cancellationToken: TestContext.Current.CancellationToken);
            await act.ShouldThrowAsync<ArgumentNullException>();

            act = () => client.Calendar.GetAllFinalesAsync(DateTime.MinValue, default, cancellationToken: TestContext.Current.CancellationToken);
            await act.ShouldThrowAsync<ArgumentNullException>();

            act = () => client.Calendar.GetAllFinalesAsync(date, default, cancellationToken: TestContext.Current.CancellationToken);
