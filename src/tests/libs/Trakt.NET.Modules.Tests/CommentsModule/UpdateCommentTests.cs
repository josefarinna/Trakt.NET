...
namespace TraktNET.CommentsModule
{
    public sealed class UpdateCommentTests
    {
        ...
        private static readonly string UpdateCommentUri = $"comments/{CommentID}";

        [Fact]
        public async Task TestTraktCommentsModuleUpdateComment()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(UpdateCommentUri, responseContent, null, null, null, null);
        ...
        public async Task TestTraktCommentsModuleUpdateCommentThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
