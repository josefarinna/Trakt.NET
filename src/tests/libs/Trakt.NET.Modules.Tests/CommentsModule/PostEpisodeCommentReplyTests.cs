...
{
    public sealed class PostEpisodeCommentReplyTests
    {
        private const string PostEpisodeCommentUri = $"comments";
        private const string CommentText = "one two three four five reply";
        private static readonly TraktEpisodeCommentPost episodeCommentPost = new()
        {
            Episode = new TraktEpisode { IDs = new TraktEpisodeIDs { Trakt = 73640U } },
            Comment = CommentText
        };

        [Fact]
        public async Task TestPostEpisodeComment()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(PostEpisodeCommentUri, responseContent, null, null, null, null);

            TraktResponse<TraktCommentPostResponse> response = await client.Comments.PostEpisodeCommentAsync(episodeCommentPost, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task TestPostEpisodeCommentWithSpoiler()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(PostEpisodeCommentUri, responseContent);
        ...
        [Fact]
        public async Task TestPostEpisodeCommentWithSharing()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        [Fact]
        public async Task TestPostEpisodeCommentComplete()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        public async Task TestPostEpisodeCommentThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(PostEpisodeCommentUri, statusCode);

            Func<Task<TraktResponse<TraktCommentPostResponse>>> act = () => client.Comments.PostEpisodeCommentAsync(episodeCommentPost, TestContext.Current.CancellationToken);
