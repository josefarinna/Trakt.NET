...
namespace TraktNET.CommentsModule
{
    public sealed class PostCommentReplyTests
    {
        ...
        [Fact]
        public async Task TestPostCommentReply()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(PostCommentReplyUri, responseContent);
        ...
        [Fact]
        public async Task TestPostCommentReplyWithSpoiler()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(PostCommentReplyUri, responseContent);

            TraktResponse<TraktCommentPostResponse> response = await client.Comments.PostCommentReplyAsync(CommentID, CommentText, Spoiler, TestContext.Current.CancellationToken);
        ...
        public async Task TestPostCommentReplyThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
