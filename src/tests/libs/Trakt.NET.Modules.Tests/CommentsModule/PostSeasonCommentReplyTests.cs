...
{
    public sealed class PostSeasonCommentReplyTests
    {
        private const string PostSeasonCommentUri = $"comments";
        private const string CommentText = "one two three four five reply";
        private readonly TraktSeasonCommentPost movieCommentPost = new()
        {
            Season = new TraktSeason { IDs = new TraktSeasonIDs { Trakt = 73640U } },
            Comment = CommentText
        };

        [Fact]
        public async Task TestPostSeasonComment()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(PostSeasonCommentUri, responseContent, null, null, null, null);

            TraktResponse<TraktCommentPostResponse> response = await client.Comments.PostSeasonCommentAsync(movieCommentPost, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task TestPostSeasonCommentWithSpoiler()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        [Fact]
        public async Task TestPostSeasonCommentWithSharing()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        [Fact]
        public async Task TestPostSeasonCommentComplete()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        public async Task TestPostSeasonCommentThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(PostSeasonCommentUri, statusCode);

            Func<Task<TraktResponse<TraktCommentPostResponse>>> act = () => client.Comments.PostSeasonCommentAsync(movieCommentPost, TestContext.Current.CancellationToken);
