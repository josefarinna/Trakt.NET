...
namespace TraktNET.CommentsModule
{
    public sealed class PostSeasonCommentReplyTests
    {
        ...
        private static readonly string PostSeasonCommentUri = $"comments";
        private static readonly TraktSeasonCommentPost seasonCommentPost = new()
        {
        ...
        [Fact]
        public async Task TestTraktCommentsModulePostSeasonComment()
        {
            ...
            TraktResponse<TraktCommentPostResponse> response = await client.Comments.PostSeasonCommentAsync(seasonCommentPost, TestContext.Current.CancellationToken);
        ...
        public async Task TestTraktCommentsModulPostSeasonCommentThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(PostSeasonCommentUri, statusCode);

            Func<Task<TraktResponse<TraktCommentPostResponse>>> act = () => client.Comments.PostSeasonCommentAsync(seasonCommentPost, TestContext.Current.CancellationToken);
