...
{
    public sealed class PostShowCommentReplyTests
    {
        private const string PostShowCommentUri = $"comments";
        private const string CommentText = "one two three four five reply";
        private readonly TraktShowCommentPost movieCommentPost = new()
        {
            Show = new TraktShow { IDs = new TraktShowIDs { Trakt = 73640U } },
            Comment = CommentText
        };

        [Fact]
        public async Task TestPostShowComment()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");

            TraktClient client = ModuleTestUtility.GetOAuthClient(PostShowCommentUri, responseContent, null, null, null, null);
            TraktResponse<TraktCommentPostResponse> response = await client.Comments.PostShowCommentAsync(movieCommentPost, TestContext.Current.CancellationToken);
        ...
        [Fact]
        public async Task TestPostShowCommentWithSpoiler()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        [Fact]
        public async Task TestPostShowCommentWithSharing()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        [Fact]
        public async Task TestPostShowCommentComplete()
        {
            string responseContent = await TestUtility.GetJsonFileContentAsync("Comments\\commentpostresponse.json");
        ...
        public async Task TestPostShowCommentThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(PostShowCommentUri, statusCode);

            Func<Task<TraktResponse<TraktCommentPostResponse>>> act = () => client.Comments.PostShowCommentAsync(movieCommentPost, TestContext.Current.CancellationToken);
