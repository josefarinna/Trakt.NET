...
namespace TraktNET.CommentsModule
{
    public sealed class PostShowCommentReplyTests
    {
        ...
        private static readonly string PostShowCommentUri = $"comments";
        private static readonly TraktShowCommentPost traktShowCommentPost = new()
        {
            Show = new TraktShow { IDs = new TraktShowIDs { Trakt = 73640U } },
            Comment = CommentText
        };
        private static TraktShowCommentPost showCommentPost = traktShowCommentPost;

        [Fact]
        public async Task TestTraktCommentsModulePostShowComment()
        {
            ...
            TraktResponse<TraktCommentPostResponse> response = await client.Comments.PostShowCommentAsync(showCommentPost, TestContext.Current.CancellationToken);
        ...
        public async Task TestTraktCommentsModulPostShowCommentThrowsAPIException(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = ModuleTestUtility.GetOAuthClient(PostShowCommentUri, statusCode);

            Func<Task<TraktResponse<TraktCommentPostResponse>>> act = () => client.Comments.PostShowCommentAsync(showCommentPost, TestContext.Current.CancellationToken);
