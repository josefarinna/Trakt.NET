...
{
    public sealed class TraktUserPersonalListItemsPostResponseTests
    {
        [Fact]
        public void TestTraktUserPersonalListItemsPostResponseDefaultConstructor()
        {
            var personalListItemsPostResponse = new TraktUserPersonalListItemsPostResponse();

            personalListItemsPostResponse.Added.ShouldBeNull();
            personalListItemsPostResponse.Existing.ShouldBeNull();
            personalListItemsPostResponse.NotFound.ShouldBeNull();
        }

        [Fact]
        public async Task TestTraktUserPersonalListItemsPostResponseFromJson()
        {
            TraktUserPersonalListItemsPostResponse? personalListItemsPostResponse = await TestUtility.DeserializeJsonAsync<TraktUserPersonalListItemsPostResponse>("Users\\userpersonallistimtespostresponse.json");

            personalListItemsPostResponse.ShouldNotBeNull();

            personalListItemsPostResponse.Added.ShouldNotBeNull();
            personalListItemsPostResponse.Added.Movies.ShouldBe(1U);
            personalListItemsPostResponse.Added.Shows.ShouldBe(2U);
            personalListItemsPostResponse.Added.Seasons.ShouldBe(3U);
            personalListItemsPostResponse.Added.Episodes.ShouldBe(4U);
            personalListItemsPostResponse.Added.People.ShouldBe(5U);

            personalListItemsPostResponse.Existing.ShouldNotBeNull();
            personalListItemsPostResponse.Existing.Movies.ShouldBe(1U);
            personalListItemsPostResponse.Existing.Shows.ShouldBe(2U);
            personalListItemsPostResponse.Existing.Seasons.ShouldBe(3U);
            personalListItemsPostResponse.Existing.Episodes.ShouldBe(4U);
            personalListItemsPostResponse.Existing.People.ShouldBe(5U);

            personalListItemsPostResponse.NotFound.ShouldNotBeNull();

            TraktUserPersonalListItemsPostResponseNotFoundGroup notFound = personalListItemsPostResponse.NotFound;
