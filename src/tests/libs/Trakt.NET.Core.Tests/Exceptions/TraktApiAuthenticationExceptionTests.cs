...
{
    public class TraktApiAuthenticationExceptionTests
    {
        [Fact]
        public async Task TestTraktApiAuthenticationExceptionCreate()
        {
            ExceptionParameters parameters = await ExceptionsTestUtility.CreateMockExceptionParametersAsync(
                Constants.StatusCodes.NotFound, HttpMethod.Get);

            var exception = new TraktApiAuthenticationException("Resource not found", parameters);
