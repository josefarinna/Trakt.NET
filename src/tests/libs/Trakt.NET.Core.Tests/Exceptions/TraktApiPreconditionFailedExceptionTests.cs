...
namespace TraktNET.Exceptions
{
    ...
    {
        [Fact]
        public async Task TestTraktApiPreconditionFailedExceptionCreate()
        {
            ExceptionParameters parameters = await ExceptionsTestUtility.CreateMockExceptionParametersAsync(
                Constants.StatusCodes.PreconditionFailed, HttpMethod.Get);

            var exception = TraktApiException.Create(parameters);

            exception.ShouldNotBeNull();
            exception.StatusCode.ShouldBe(Constants.StatusCodes.PreconditionFailed);
            exception.ReasonPhrase.ShouldBe("Precondition Failed - use application/json content type");
            ...
            exception.Message.ShouldBe("Trakt API request failed. Precondition Failed - use application/json content type");
        }
    }
}
