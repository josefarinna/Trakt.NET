...
namespace TraktNET.PostRequests.Users
{
    public sealed class UserReportPostRequestTests
    {
        ...
        {
            var userReportPostRequest = new UserReportPostRequest { Id = default! };
        ...
        {
            var userReportPostRequest = new UserReportPostRequest { Id = string.Empty };
            Action act = () => userReportPostRequest.Validate();
            act.ShouldThrow<TraktRequestValidationException>();

            userReportPostRequest = new UserReportPostRequest { Id = "  " };
            act = () => userReportPostRequest.Validate();
            act.ShouldThrow<TraktRequestValidationException>();

            userReportPostRequest = new UserReportPostRequest { Id = "id with spaces" };
            act = () => userReportPostRequest.Validate();
            act.ShouldThrow<TraktRequestValidationException>();
        }
    }
