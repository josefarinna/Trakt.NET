...
{
    public sealed class ListsTrendingGetRequestTests
    {
        private const string URIPath = "lists/trending";
        ...
        public void TestListsTrendingGetRequestHasValidURIPath(TraktListType? type, TraktExtendedInfo? extendedInfo, int? page, int? limit, string expectedURIPath)
        {
            var listsTrendingGetRequest = new ListsTrendingGetRequest
            {
                Type = type,
            ...
            listsTrendingGetRequest.BuildUri();
            listsTrendingGetRequest.RequestUri.ShouldBe(new Uri(expectedURIPath, UriKind.Relative));
        }

        [Fact]
        public void TestListsTrendingGetRequestHasValidOAuthRequirement()
        {
            var listsTrendingGetRequest = new ListsTrendingGetRequest();
            listsTrendingGetRequest.OAuthRequirement.ShouldBe(TraktOAuthRequirement.NotRequired);
        }

        [Fact]
        public void TestListsTrendingGetRequestIsGetRequest()
        {
            var listsTrendingGetRequest = new ListsTrendingGetRequest();
            listsTrendingGetRequest.Method.ShouldBe(HttpMethod.Get);
        }

        [Fact]
        public void TestListsTrendingGetRequestHasCorrectRequestObjectType()
        {
            var listsTrendingGetRequest = new ListsTrendingGetRequest();
            listsTrendingGetRequest.RequestObjectType.ShouldBe(TraktRequestObjectType.List);
        }
    }
}
