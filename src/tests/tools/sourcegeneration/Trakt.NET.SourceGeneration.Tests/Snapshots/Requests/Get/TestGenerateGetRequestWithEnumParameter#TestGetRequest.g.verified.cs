//-----------------------------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a Trakt.NET source generator.
//     Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.
// </auto-generated>
//-----------------------------------------------------------------------------------------------------

#nullable enable

namespace SourceGeneraterTestNamespace
{
    internal sealed partial class TestGetRequest : RequestBase
    {
        internal required TraktReactionType Type { get; init; }

        internal override TraktOAuthRequirement OAuthRequirement => TraktOAuthRequirement.NotRequired;

        internal TestGetRequest() : base(HttpMethod.Get, (Uri?)null) { }

        internal override void BuildUri()
        {
            string requestUri = $"comments/{Type.AsPathParameter()}";
            RequestUri = new Uri(requestUri, UriKind.Relative);
        }

        internal override void Validate()
        {
            if (Type == TraktReactionType.Unspecified)
            {
                throw new TraktRequestValidationException(nameof(Type), "Type must not be Unspecified");
            }
        }
    }
}
