namespace TraktNET
{
    public sealed partial class TraktShowsModule
    {
        ...
        /// <para><see href="https://docs.trakt.tv/reference/postshowsepisodereport">
        /// Trakt API Documentation: Shows: Report an episode
        /// </see></para>
        /// </remarks>
        /// <exception cref="TraktApiException">Thrown if the request fails.</exception>
        /// <exception cref="TraktRequestValidationException">Thrown if the validation (e.g. invalid id) of the request fails.</exception>
        public Task<TraktResponse> ReportShowEpisodeAsync(string traktShowIDOrSlug, uint seasonNumber, uint episodeNumber, TraktReason reason, string? message = null, CancellationToken cancellationToken = default)
            => ReportShowEpisodeImplAsync(traktShowIDOrSlug, seasonNumber, episodeNumber, reason, message, cancellationToken);
        ...
        public Task<TraktResponse> ReportShowEpisodeAsync(uint traktShowID, uint seasonNumber, uint episodeNumber, TraktReason reason, string? message = null, CancellationToken cancellationToken = default)
        {
            if (traktShowID == 0)
                throw new ArgumentException("show id must not be 0", nameof(traktShowID));

            return ReportShowEpisodeAsync(traktShowID.ToInvariantCultureString(), seasonNumber, episodeNumber, reason, message, cancellationToken);
        }
        ...
        public Task<TraktResponse> ReportShowEpisodeAsync(TraktShowIDs showIDs, uint seasonNumber, uint episodeNumber, TraktReason reason, string? message = null, CancellationToken cancellationToken = default)
        {
            ...
            return ReportShowEpisodeAsync(showIDs.BestID, seasonNumber, episodeNumber, reason, message, cancellationToken);
        }
    }
}
