namespace TraktNet.Objects.Get.Shows.Json.Writer
{
    ...
    internal class AShowObjectJsonWriter<TShowObjectType> : AObjectJsonWriter<TShowObjectType> where TShowObjectType : ITraktShow
    {
        public override async Task WriteObjectAsync(JsonTextWriter jsonWriter, TShowObjectType obj, CancellationToken cancellationToken = default)
        {
            CheckJsonTextWriter(jsonWriter);
            await jsonWriter.WriteStartObjectAsync(cancellationToken).ConfigureAwait(false);
            await WriteShowObjectAsync(jsonWriter, obj, cancellationToken).ConfigureAwait(false);
            await jsonWriter.WriteEndObjectAsync(cancellationToken).ConfigureAwait(false);
        }

        protected virtual async Task WriteShowObjectAsync(JsonTextWriter jsonWriter, TShowObjectType obj, CancellationToken cancellationToken = default)
        {
            ...
            if (obj.Airs != null)
            {
                ...
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_AIRS, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_AIRED_EPISODES, cancellationToken).ConfigureAwait(false);
