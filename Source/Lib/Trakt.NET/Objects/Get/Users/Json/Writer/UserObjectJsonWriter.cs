namespace TraktNet.Objects.Get.Users.Json.Writer
{
    ...
    internal class UserObjectJsonWriter : AObjectJsonWriter<ITraktUser>
    {
        public override async Task WriteObjectAsync(JsonTextWriter jsonWriter, ITraktUser obj, CancellationToken cancellationToken = default)
        {
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_USERNAME, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.Username, cancellationToken).ConfigureAwait(false);
            }

            if (obj.IsPrivate.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_PRIVATE, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_VIP, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.IsVIP, cancellationToken).ConfigureAwait(false);
            }

            if (obj.IsVIP_EP.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_VIP_EP, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.IsVIP_EP, cancellationToken).ConfigureAwait(false);
            }

            if (obj.JoinedAt.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_JOINED_AT, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_LOCATION, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.Location, cancellationToken).ConfigureAwait(false);
            }

            if (!string.IsNullOrEmpty(obj.About))
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_ABOUT, cancellationToken).ConfigureAwait(false);
            ...
            if (obj.Age.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_AGE, cancellationToken).ConfigureAwait(false);
