namespace TraktNet.Objects.Get.Tests.History.Json.Writer
{
    ...
    public partial class HistoryItemObjectJsonWriter_Tests
    {
        ...
        [Fact]
        public async Task Test_HistoryItemObjectJsonWriter_Episode_WriteObject_JsonWriter_Only_ID_Property()
        {
            ITraktHistoryItem traktHistoryItem = new TraktHistoryItem
            {
                Id = 1982347UL
        ...
        [Fact]
        public async Task Test_HistoryItemObjectJsonWriter_Episode_WriteObject_JsonWriter_Only_WatchedAt_Property()
        {
            ITraktHistoryItem traktHistoryItem = new TraktHistoryItem
            {
        ...
        [Fact]
        public async Task Test_HistoryItemObjectJsonWriter_Episode_WriteObject_JsonWriter_Only_Action_Property()
        {
            ITraktHistoryItem traktHistoryItem = new TraktHistoryItem
            {
        ...
        [Fact]
        public async Task Test_HistoryItemObjectJsonWriter_Episode_WriteObject_JsonWriter_Only_Type_Property()
        {
            ITraktHistoryItem traktHistoryItem = new TraktHistoryItem
            {
            ...
            using (var jsonWriter = new JsonTextWriter(stringWriter))
            {
                var traktJsonWriter = new HistoryItemObjectJsonWriter();
                await traktJsonWriter.WriteObjectAsync(jsonWriter, traktHistoryItem);
                stringWriter.ToString().Should().Be(@"{""id"":0,""type"":""episode""}");
            }
        }

        [Fact]
        public async Task Test_HistoryItemObjectJsonWriter_Episode_WriteObject_JsonWriter_Only_Episode_Property()
        {
            ITraktHistoryItem traktHistoryItem = new TraktHistoryItem
            {
            ...
            using (var jsonWriter = new JsonTextWriter(stringWriter))
            {
                var traktJsonWriter = new HistoryItemObjectJsonWriter();
                await traktJsonWriter.WriteObjectAsync(jsonWriter, traktHistoryItem);
                stringWriter.ToString().Should().Be(@"{""id"":0,""episode"":{""season"":1,""number"":1,""title"":""Winter Is Coming""," +
                                                    @"""ids"":{""trakt"":73640,""tvdb"":3254641,""imdb"":""tt1480055"",""tmdb"":63056,""tvrage"":1065008299}}}");
            }
        }

        [Fact]
        public async Task Test_HistoryItemObjectJsonWriter_Episode_WriteObject_JsonWriter_Only_Show_Property()
        {
            ITraktHistoryItem traktHistoryItem = new TraktHistoryItem
            {
                Show = new TraktShow
