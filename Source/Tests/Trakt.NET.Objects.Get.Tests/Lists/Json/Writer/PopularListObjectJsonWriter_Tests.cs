namespace TraktNet.Objects.Get.Tests.Lists.Json.Writer
{
    ...
    public class PopularPopularListObjectJsonWriter_Tests
    {
        private readonly DateTime CREATED_AT = DateTime.UtcNow;
        private readonly DateTime UPDATED_AT = DateTime.UtcNow;

        [Fact]
        public async Task Test_PopularListObjectJsonWriter_WriteObject_Exceptions()
        {
            var traktJsonWriter = new PopularListObjectJsonWriter();
            Func<Task<string>> action = () => traktJsonWriter.WriteObjectAsync(default);
            await action.Should().ThrowAsync<ArgumentNullException>();
        }

        [Fact]
        public async Task Test_PopularListObjectJsonWriter_WriteObject_Empty()
        {
            var traktJsonWriter = new PopularListObjectJsonWriter();
            string json = await traktJsonWriter.WriteObjectAsync(new TraktPopularList());
            json.Should().Be("{}");
        }

        [Fact]
        public async Task Test_PopularListObjectJsonWriter_WriteObject_Only_LikeCount_Property()
        {
            ITraktPopularList popularList = new TraktPopularList
            {
                LikeCount = 5
            };

            var traktJsonWriter = new PopularListObjectJsonWriter();
            string json = await traktJsonWriter.WriteObjectAsync(popularList);
            json.Should().Be(@"{""like_count"":5}");
        }

        [Fact]
        public async Task Test_PopularListObjectJsonWriter_WriteObject_Only_CommentCount_Property()
        {
            ITraktPopularList popularList = new TraktPopularList
            {
                CommentCount = 5
        ...
        [Fact]
        public async Task Test_PopularListObjectJsonWriter_WriteObject_Only_List_Property()
        {
            ITraktPopularList popularList = new TraktPopularList
            {
                List = new TraktList
        ...
        [Fact]
        public async Task Test_PopularListObjectJsonWriter_WriteObject_Complete()
        {
            ITraktPopularList popularList = new TraktPopularList
            {
                LikeCount = 5,
