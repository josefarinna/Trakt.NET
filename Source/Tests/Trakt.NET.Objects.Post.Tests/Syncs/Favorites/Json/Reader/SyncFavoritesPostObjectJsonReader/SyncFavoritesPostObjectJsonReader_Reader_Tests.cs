namespace TraktNet.Objects.Post.Tests.Syncs.Favorites.Json.Reader
{
    ...
    {
        [Fact]
        public async Task Test_SyncFavoritesPostObjectJsonReader_ReadObject_From_JsonReader_Complete()
        {
            var traktJsonReader = new SyncFavoritesPostObjectJsonReader();

            using var reader = new StringReader(JSON_COMPLETE);
            using var jsonReader = new JsonTextReader(reader);
            ITraktSyncFavoritesPost traktSyncFavoritesPost = await traktJsonReader.ReadObjectAsync(jsonReader);

            traktSyncFavoritesPost.Should().NotBeNull();
            traktSyncFavoritesPost.Movies.Should().NotBeNull().And.HaveCount(2);

            ITraktSyncFavoritesPostMovie[] postMovies = traktSyncFavoritesPost.Movies.ToArray();
            ...
            traktSyncFavoritesPost.Shows.Should().NotBeNull().And.HaveCount(2);

            ITraktSyncFavoritesPostShow[] postShows = traktSyncFavoritesPost.Shows.ToArray();
        ...
        [Fact]
        public async Task Test_SyncFavoritesPostObjectJsonReader_ReadObject_From_JsonReader_Null()
        {
            var traktJsonReader = new SyncFavoritesPostObjectJsonReader();
            Func<Task<ITraktSyncFavoritesPost>> traktSyncFavoritesPost = () => traktJsonReader.ReadObjectAsync(default(JsonTextReader));
            await traktSyncFavoritesPost.Should().ThrowAsync<ArgumentNullException>();
        }

        [Fact]
        public async Task Test_SyncFavoritesPostObjectJsonReader_ReadObject_From_JsonReader_Empty()
        {
            var traktJsonReader = new SyncFavoritesPostObjectJsonReader();
            ...
            using var jsonReader = new JsonTextReader(reader);
            ITraktSyncFavoritesPost traktSyncFavoritesPost = await traktJsonReader.ReadObjectAsync(jsonReader);
            traktSyncFavoritesPost.Should().BeNull();
        }
    }
}
