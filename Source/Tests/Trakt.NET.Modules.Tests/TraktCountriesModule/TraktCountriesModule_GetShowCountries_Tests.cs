namespace TraktNet.Modules.Tests.TraktCountriesModule
{
    ...
    public partial class TraktCountriesModule_Tests
    {
        ...
        public async Task Test_TraktCountriesModule_GetShowCountries_Throws_API_Exception(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = TestUtility.GetMockClient(COUNTRIES_SHOWS_URI, statusCode);

            try
            {
                await client.Countries.GetShowCountriesAsync();
                Assert.False(true);
            }
