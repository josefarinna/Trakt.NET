namespace TraktNet.Modules.Tests.TraktCountriesModule
{
    ...
    [Trait("Category", "Modules.Countries")]
    public partial class TraktCountriesModule_Tests
    {
        ...
        public async Task Test_TraktCountriesModule_GetMovieCountries_Throws_API_Exception(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = TestUtility.GetMockClient(COUNTRIES_MOVIES_URI, statusCode);

            try
            {
                await client.Countries.GetMovieCountriesAsync();
                Assert.False(true);
            }
