namespace TraktNet.Modules.Tests.TraktLanguagesModule
{
    ...
    [Trait("Category", "Modules.Languages")]
    public partial class TraktLanguagesModule_Tests
    {
        ...
        public async Task Test_TraktLanguagesModule_GetMovieLanguages_Throws_API_Exception(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = TestUtility.GetMockClient(LANGUAGES_MOVIES_URI, statusCode);

            try
            {
                await client.Languages.GetMovieLanguagesAsync();
                Assert.False(true);
            }
