namespace TraktNet.Modules.Tests.TraktCalendarModule
{
    ...
    public partial class TraktCalendarModule_Tests
    {
        ...
        public async Task Test_TraktCalendarModule_GetUserShows_Throws_API_Exception(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = TestUtility.GetOAuthMockClient(GET_USER_SHOWS_URI, statusCode);

            try
            {
                await client.Calendar.GetUserShowsAsync();
                Assert.False(true);
            }
