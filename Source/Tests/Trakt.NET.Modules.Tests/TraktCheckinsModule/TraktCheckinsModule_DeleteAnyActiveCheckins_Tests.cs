namespace TraktNet.Modules.Tests.TraktCheckinsModule
{
    ...
    [Trait("Category", "Modules.Checkins")]
    public partial class TraktCheckinsModule_Tests
    {
        ...
        public async Task Test_TraktCheckinsModule_DeleteAnyActiveCheckins_Throws_API_Exception(HttpStatusCode statusCode, Type exceptionType)
        {
            TraktClient client = TestUtility.GetOAuthMockClient(CHECKIN_URI, statusCode);

            try
            {
                await client.Checkins.DeleteAnyActiveCheckinsAsync();
                Assert.False(true);
            }
