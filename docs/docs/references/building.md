...
```ps
dotnet build src/Trakt.NET.slnx -c [Debug|Release]
```

Running the tests:
```ps
dotnet test src/Trakt.NET.slnx -c [Debug|Release] --no-build --no-restore
```

Alternatively you can also open the solution `<PROJECT_ROOT>/src/Trakt.NET.slnx` in Visual Studio.
